...
class IMS:
    def __init__(self, root):
        self.root = root
        self.root.geometry("1350x700+30+0")
        self.root.title("warehouse managment")
        self.root.config(bg="gray")
        # --------------title
        self.icon_title = PhotoImage(file="images\cart.png")

        title = Label(self.root, text="WELCOME TO WAREHOUSE MANAGEMENT", image=self.icon_title, compound=LEFT, font=(
        ...
        #     "times new roman", 20, "bold"), bg="light blue", bd=3, cursor="hand2").pack(side=TOP, fill=X)
        btn_exit = Button(LeftMenu, text="exit", command=self.exit, image=self.icon_side, compound=LEFT, padx=5, anchor="w", font=(
            "times new roman", 20, "bold"), bg="light blue", bd=3, cursor="hand2").pack(side=TOP, fill=X)

        # --------------------content
        self.lbl_employee = Label(self.root, text="total employee", bd=5, relief=RAISED,
                                  bg="green", fg="white", font=("goudy old style", 20, "bold"))
        self.lbl_employee.place(x=300, y=120, height=150, width=300)

        self.lbl_supplier = Label(self.root, text="total supplier", bd=5, relief=RAISED,
